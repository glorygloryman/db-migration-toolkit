...
skills/                          ← 10 个 Claude Code Skills，对应 Stage 0/0/1/1/1/3/4/4/4/5
...
├── db-migration-stage4-plan-rewrite  ← Stage 4: 将改写清单拆解为原子任务执行计划
├── db-migration-stage4-execute-task  ← Stage 4: 逐条执行改写任务 + 跑测试 + 回写进度
...
Skill 统一结构：触发场景 → 前置条件 → 执行步骤 → 输出 → 约束 → 后续步骤。骨架期步骤多为大纲，Pilot 后精化为可执行指令。大部分 Skill 产出**只写文档、不改代码**（`db-migration-dialect-rewrite` 也只出 diff 建议，由人工 apply）。例外：`db-migration-test-execute` 会编写测试代码但不修改业务代码；`db-migration-stage4-execute-task` 会执行方言改写但严格遵循源文档建议方案。
