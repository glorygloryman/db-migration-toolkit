...
5. **集成测试必须使用真实 schema 对象**：测试代码、`@Sql`、`@BeforeAll`、fixture、support helper、验证工具不得自行执行 `CREATE DATABASE` / `CREATE SCHEMA` / `CREATE TABLE` / `CREATE TEMPORARY TABLE` / `CREATE TABLE ... LIKE ...` / `ALTER TABLE` / `DROP TABLE` / `DROP TEMPORARY TABLE` 等数据库对象 DDL；测试数据准备只允许对真实库中已存在的表做 `INSERT` / `UPDATE` / `DELETE` 并清理
6. **缺失库、表、字段必须早暴露**：Stage 0 / Stage 1 必须对代码引用的库、表、字段与真实 MySQL schema 做差异检查，发现缺失立即记为 blocker；禁止通过临时库、临时表、影子表、fixture 表绕过，Stage 4 只处理方言差异，不承担首次发现 schema 缺口
7. **Flyway 严禁改历史脚本**，新建 `db/migration/highgo/` 目录（与全局 §3.6 一致）
...
Skill 统一结构：触发场景 → 前置条件 → 执行步骤 → 输出 → 约束 → 后续步骤。骨架期步骤多为大纲，Pilot 后精化为可执行指令。大部分 Skill 产出**只写文档、不改代码**（`db-migration-dialect-rewrite` 也只出 diff 建议，由人工 review 后经 superpowers 流程执行）。例外：`db-migration-test-execute` 会编写测试代码但不修改业务代码。
