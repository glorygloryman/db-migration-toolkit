...
- 使用真实 MySQL schema 中已存在的库、表、字段
- **禁止引入 Testcontainers 依赖**
- **禁止使用 `@MockBean` 替代数据库**
- **禁止自行创建数据库对象**：测试代码、`@Sql`、`@BeforeAll`、fixture、support helper 中不得执行 `CREATE DATABASE`、`CREATE SCHEMA`、`CREATE TABLE`、`CREATE TEMPORARY TABLE`、`CREATE TABLE ... LIKE ...`、`ALTER TABLE`、`DROP TABLE`、`DROP TEMPORARY TABLE`
- 使用 `@Sql` 或代码方式准备测试数据时，只允许对真实 schema 中已存在的表执行 `INSERT` / `UPDATE` / `DELETE`
- 测试后清理数据（`@Sql` cleanup 或 `@Transactional` rollback）
- 若测试所需库、表或字段不存在，停止编写通过性测试，将该 Task 标记为 schema blocker；禁止通过临时库、临时表、影子表或 fixture 表绕过
- 验证 SQL 执行结果与预期一致
- 断言消息使用中文
...
- **禁止引入 Testcontainers 依赖**
- 集成测试必须使用真实 schema 中已存在的库、表、字段
- 禁止测试自行创建数据库对象：不得执行 `CREATE DATABASE`、`CREATE SCHEMA`、`CREATE TABLE`、`CREATE TEMPORARY TABLE`、`CREATE TABLE ... LIKE ...`、`ALTER TABLE`、`DROP TABLE`、`DROP TEMPORARY TABLE`
- 测试数据准备只允许对真实表执行 `INSERT` / `UPDATE` / `DELETE`，并清理测试数据
- 若真实 schema 缺失库、表或字段，必须标记 blocker，不允许通过临时对象让测试通过
- 每次只执行一个 Task
...
- 集成测试的数据准备和清理是否完整
- 集成测试是否只使用真实 schema 中已存在的库、表、字段
- 是否存在测试自行创建数据库对象或临时库、临时表、影子表、fixture 表
